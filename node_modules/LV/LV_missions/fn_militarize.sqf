...
_menArray = ([_classModuleFilters,[(_side), 6]] call LV_fnc_classnames);
_menArray = [_menArray] call LV_fnc_validateClassArrays;
if((count _menArray) == 0)then{
	_menArray = ([[],[(_side), 6]] call LV_fnc_classnames);
};
_menArray = selectRandom _menArray;
_diveArray = ([_classModuleFilters,[(_side), 7]] call LV_fnc_classnames);
_diveArray = [_diveArray] call LV_fnc_validateClassArrays;
if((count _diveArray) == 0)then{
	_diveArray = ([[],[(_side), 7]] call LV_fnc_classnames);
};
	...
	for "_i" from 1 to _menAmount do{
		...
		if(!_still)then{
			if(_unitType in _menArray)then{
				[_unit,_cPos,_radius,_doorHandling] call LV_fnc_patrol-vD.sqf;
			}else{
				[_unit,_pos] call LV_fnc_patrol-vH.sqf;
			};
		};
...
if((_vehicles select 0)||(_vehicles select 1)||(_vehicles select 2))then{
			...
			if(true)then{ //side > 0
				if(surfaceIsWater _pos)then{
					if(_vehicles select 1)then{
						_driver = [_pos, (_side),_classModuleFilters,nil,nil,false] call LV_fnc_fullWaterVehicle;
						if(!_still)then{[vehicle _driver,_pos] call LV_fnc_patrol-vH.sqf;};
						_validPos = true;
					}else{
						_wMode = 0;
					};
					...
					if((_vehicles select 0)&&(_vehicles select 2))then{
						...
						if(_whichOne < 3)then{
							_driver = [_pos, (_side),_classModuleFilters,nil,nil,false] call LV_fnc_fullAirVehicle;
							if(!_still)then{[_driver,_pos,[200,200]] call LV_fnc_patrol-vE.sqf;};
							vehicle _driver flyInHeight 10;
							_validPos = true;
						}else{
							_driver = [_pos, (_side),_classModuleFilters,nil,nil,false] call LV_fnc_fullLandVehicle;
							if(!_still)then{[vehicle _driver,_pos] call LV_fnc_patrol-vE.sqf;};
							_validPos = true;
						};
					}else{
	...
	{ 
		if((typeName _skills != "STRING")&&((side _x) != civilian))then{ _skls = [_x,_skills] call LV_fnc_ACskills; }; 
		if(!isNil("_customInit"))then{ 
			[_x,_customInit] spawn LV_fnc_vehicleInit;
		};
...
if(_smokesAndChems)then{
	[{
		params ["_grp", "_handle"];
		if (({alive _x}count units _grp) > 0) exitWith {[_handle] call CBA_fnc_removePerFrameHandler;};
		{
	...
	},10,_milGroup] call CBA_fnc_addPerFrameHandler;
};

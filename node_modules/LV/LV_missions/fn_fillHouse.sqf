...
private ["_blueMenArray3","_blueMenArray2","_BLUarrays","_redMenArray2","_OPFarrays","_greenMenArray","_grpId","_customInit","_center","_skls","_skills","_a","_buildings","_rat","_milHQ","_milGroup","_menArray","_i","_newPos","_i2","_unitType","_unit","_building","_sideOption","_blueMenArray","_redMenArray","_bPoss","_patrol","_pFile","_pType","_input","_logic","_isActivated","_mRatio","_mGroup","_classModule","_classModuleFilters"];
...
_menArray = ([_classModuleFilters,[(_sideOption), 6]] call LV_fnc_classnames);
_menArray = [_menArray] call LV_fnc_validateClassArrays;
if((count _menArray) == 0)then{
	_menArray = ([[],[(_sideOption), 6]] call LV_fnc_classnames);
};
...
if(_radius > 1)then{
	_buildings = ["all in radius",_center,_radius] call LV_fnc_nearestBuilding;
}else{
	_buildings = ["nearest one",_center] call LV_fnc_nearestBuilding;
};
...
while{_i2 < _rat}do{
	...
	if(typeName _skills != "STRING")then{_skls = [_unit,_skills] call LV_fnc_ACskills;};
	...
	if(_patrol)then{
			...
			[_unit] execVM format["%1",_pFile]; 
	}else{
        	doStop _unit;
    };
	if(!isNil("_customInit"))then{ 
		nul = [_unit,_customInit] spawn LV_fnc_vehicleInit;
	};
};

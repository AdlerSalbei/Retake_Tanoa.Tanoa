...
_choppers = ([_classModuleFilters,[(_side), 3]] call LV_fnc_classnames);
_chopperTypes = [];
...
_chopperTypes = [_choppers] call LV_fnc_validateClassArrays;
if((count _chopperTypes) == 0)then{
	_chopperTypes = ([[],[(_side), 3]] call LV_fnc_classnames);
};
_chopperTypes = selectRandom _chopperTypes;

_men = ([_classModuleFilters,[(_side), 6]] call LV_fnc_classnames);
_men = [_men] call LV_fnc_validateClassArrays;
if((count _men) == 0)then{
	_men = ([[],[(_side), 6]] call LV_fnc_classnames);
};
...
if(!_exactPos)then{
	...
	while{_finding > 0}do{
			...
			while{count _tPos < 1}do{
					//_spot = [_targetPos, _ra] call LV_fnc_randomSpot;
...
for "_i" from 1 to _vehSpots do {
	...
	if(typeName _skills != "STRING")then{_skls = [_man2,_skills] call LV_fnc_ACskills;};
	_man2 moveInCargo _heli;
	if(!isNil("_customInit"))then{ 
		[_man2,_customInit] spawn LV_fnc_vehicleInit;
	};
};
